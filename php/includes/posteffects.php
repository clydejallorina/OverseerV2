...
while ($i <= $n) {
    ...
    while (!empty($status[$j])) { //We've found a status entry
        ...
        if ($currentstatus[1] == "1") {
            $removed = true;
        } //Duration 1: This status just expired
    ...
    while (!empty($bonus[$j])) { //We've found a bonus entry
        ...
        if ($currentbonus[1] == "1") {
            $removed = true;
        } //Duration 1: This bonus just expired
        ...
        switch ($abilities[$j]) {
            case "12": //Battle Fury (ID 12)
                if ($strifers[$i]['damagetaken'] > 0) { //Strifer took damage in combat this round: Battle Fury triggers
                    ...
                    if ($offenseboost > ($strifers[$i]['maxpower'] / 8)) {
                        $offenseboost = ($strifers[$i]['maxpower'] / 8);
                    }
            ...
            case "-8": //Metis's post-round ability.
                ...
                if ($strifers[$i]['subaction'] == 0 && $strifers[$i]['energy'] >= $cost) { //Didn't use another action, so we can do this.
                    ...
                    while (!empty($strifers[$k])) {
                        if ($strifers[$k]['side'] != $strifers[$i]['side']) {
                            $enemies++;
                        }
                    }
                    ...
                    if ($decision > 0) {
                        ...
                        switch ($decision) {
                            ...
                            case $offensedown:
                                ...
                                while (!empty($strifers[$k])) {
                                    if ($strifers[$k]['side'] != $strifers[$i]['side']) {
                                        $strifers[$k]['bonuses'] .= "OFFENSE:0:-413|OFFENSE:3:-413|";
                                    }
                                }
                                break;
                            case $defensedown:
                                ...
                                while (!empty($strifers[$k])) {
                                    if ($strifers[$k]['side'] != $strifers[$i]['side']) {
                                        $strifers[$k]['bonuses'] .= "DEFENSE:0:-413|DEFENSE:3:-413|";
                                    }
                                }
                                break;
                            case $actiondisable:
            ...
            case "Life/I":
                ...
                while ($j <= $n) {
                    if ($strifers[$j]['side'] == $strifers[$i]['side']) {
                        $strifers[$j]['health'] += ceil($strifers[$j]['maxhealth'] * 0.413);
                    }
