...
class Code
{
     ...
     */
    private function binary(string $char): int
    {
        $c = ord($char);
     ...
     * @return 0|string
     */
    private function character(int $bin): int|string
    {
        if ($bin >= 0 && $bin <= 9) {
            return chr(ord("0") + $bin);
        }
        if ($bin >= 10 && $bin <= 35) {
            return chr(ord("A") + $bin - 10);
        }
        if ($bin >= 36 && $bin <= 61) {
            return chr(ord("a") + $bin - 36);
        }
        if ($bin == 62) {
            return "?";
        }
